...
* Copyright 2017 http://www.klsst.com/ All rights reserved.
...
jQuery.kaddFavorite.js
jQuery.ksethome.js
jQuery.ksidenav.js
jQuery.khover.js v2.0
jQuery.kfocusblur.js V2.0 
jQuery.kselect.js	下拉
jQuery.ktab.js v2.0 选项卡
jQuery.kfold.js v2.0 折叠
jQuery.ktags.js v2.0 选项卡
jQuery.ktagsmixed.js
jQuery.ktextarealength.js v2.0
jQuery.kpreview.js v2.0
jQuery.kmodalalert.js
jQuery.kalert.js
jQuery.ktotop.js v2.0
jQuery.kmarquee.js
jQuery.kspinner.js v2.0
...
/* =======================================================================
 * jQuery.kspinner.js v2.1.2 微调器
 * http://www.h-ui.net/
 * ========================================================================*/
!function($) {	
	$.fn.kspinner = function(options, callback) {
		var defaults = {
			value : 1,
			minValue : 1,
			maxValue : 999,
...
/* =======================================================================
 * jquery.kaddFavorite.js 添加收藏
 * ========================================================================*/
function kaddFavorite(obj) {
	...
	try {
		...
		try {
			...
			$.kmodalalert("加入收藏失败，请使用Ctrl+D进行添加", 2000);
		}
	}
}

/* ========================================================================
 * jQuery.ksethome.js 设为首页
 * ======================================================================== */
function ksethome(obj){
	try{
...
/* =======================================================================
 * jQuery.kfocusblur.js v2.0 得到失去焦点
 * ========================================================================*/
!function($) {
	$.fn.kfocusblur = function(options){
		var defaults = {
			className:"focus",
		}
		var options = $.extend(defaults, options);
...
/* =======================================================================
 * jQuery.kselect.js 选择
 * ========================================================================*/
!function($) {
	$.kselect = function(divselectid, inputselectid) {
 ...
 * ========================================================================*/
!function($) {
	$.fn.kfold = function(options){
		var defaults = {
			titCell:'.item .kfold-header',
			mainCell:'.item .kfold-body',
		...
		this.each(function(){	
			...
			if(options.triggerType){
				$(options.titCell).children("i:last-child").on(options.trigger,function(){
					var that_ = $(this).parent();
					if (that_.next().is(":visible")) {
						if (options.type == 2) {
							return false;
						} else {
							that_.next().slideUp(options.speed).end().removeClass(options.className);
							if (that_.find("b")) {
								that_.find("b").html("+");
							}
						}
						...
						if (options.type == 3) {
							that_.next().slideDown(options.speed).end().addClass(options.className);
							if (that_.find("b")) {
								that_.find("b").html("-");
							}
						} else {
							that_.find(options.mainCell).slideUp(options.speed);
							that_.find(options.titCell).removeClass(options.className);
							if (that_.find(options.titCell).find("b")) {
								that_.find(options.titCell).find("b").html("+");
							}
							that_.next().slideDown(options.speed).end().addClass(options.className);
							if (that_.find("b")) {
								that_.find("b").html("-");
...
!function($) {
	$.fn.ktags = function(options) {
		...
		this.each(function(){
			var that = $(this);
			var str = 
			'<div class="ktags-wraper">'+
				'<div class="ktags-editor cl"></div>'+
				'<div class="ktags-input-wraper">'+
					'<input type="text" class="input-text ktags-input" maxlength="'+options.maxlength+'" value="">'+
				'</div>'+
				'<div class="ktags-list">'+
					'<div class="ktags-notag" style="display:none">暂无常用标签</div>'+
					'<div class="ktags-has"></div>'+
				'</div>'+
				'<input type="hidden" class="ktags-val" name="" value="'+options.value+'">'+
			'</div>';
			that.append(str);
			var wraper = that.find(".ktags-wraper");
			var editor = that.find(".ktags-editor");
			var input =that.find(".ktags-input");
			var list = that.find(".ktags-list");
			var has = that.find(".ktags-has");
			var val = that.find(".ktags-val");
			...
			if(options.tagsDefault){
				...
				has.find("span").on('click',function(){
					...
					editor.append('<span class="ktags-token">'+taghasV+'</span>');
					gettagval(this);
					$(this).remove();
				});
			}
			...
			function gettagval(obj) {
				var str = "";
				var token = that.find(".ktags-token");
				if (token.length < 1) {
					input.val("");
					return false;
				}
			...
			input.on("change",function(){
				...
				var v = v2+','+v1;
				if(v!=''){
					var str='<i class="ktags-icon iconfont">&#xe64b;</i>';
					var result = unique(v, 1);
					if(result.length>0){
						for(var j=0;j<result.length;j++){
							str+='<span class="ktags-token">'+result[j]+'</span>';
						}
						val.val(result);
						editor.html(str);
						input.val("").blur();
			...
			$(document).on("click",".ktags-token",function(){
				$(this).remove();
				var str ="";
				if(that.find(".ktags-token").length<1){
					val.val("");
					return false;
				}else{
					for(var i = 0;i< that.find(".ktags-token").length;i++){
						str += that.find(".ktags-token").eq(i).text() + ",";
					}
...
/* =======================================================================
 * jQuery.ktagsmixed.js 标签混排效果
 * ========================================================================*/
!function($) {
	$.ktagsmixed = function(obj) {
		$(obj).each(function() {
			var x = 9;
			var y = 0;
...
!function($) {
	$.fn.ktextarealength = function(options){
		var defaults = {
			minlength:0,
			maxlength:140,
			errorClass:"error",
...
/* =======================================================================
 * jQuery.kmodalalert.js alert
 * ========================================================================*/
!function($) {
	$.kmodalalert = function(info, speed) {
		...
		if (speed == 0 || typeof(speed) == "undefined") {
			...
			$("#modal-alert").fadeIn();
			setTimeout($.kmodalalert.hide, speed);
		}
	}
	$.kmodalalert.hide = function() {
...
/* =======================================================================
 * jQuery.kalert.js alert
 * ========================================================================*/
!function($) {
	$.kalert = function() {
		$(".kalert i").khover();
		$(".kalert i").on("click",function() {
			var kalert = $(this).parents(".kalert");
			kalert.fadeOut("normal",
			function() {
				kalert.remove();
			});
		});
...
/* =======================================================================
 * jQuery.ktotop.js v2.0 返回顶部
 * ========================================================================*/
!function($) {
	//bottom 距离底部高度
	$.ktotop = function(bottom){		
		if(!bottom){
			bottom = 60;
		}
		var str ='<a href="javascript:void(0)" class="tools-right toTop iconfont" title="返回顶部" alt="返回顶部" style="display:none;bottom:'+bottom+'px">&#xe684;</a>';
...
/* =======================================================================
 * jQuery.kmarquee.js 滚动
 * ========================================================================*/
!function($) {
	$.kmarquee = function(height, speed, delay) {
		var scrollT;
		var pause = false;
