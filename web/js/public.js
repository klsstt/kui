...
//判断数字和字母
function checkInputName(id, errId, minlength, maxlength, tipRequire, tipRegx, tipSpecial) {
	var str = $.trim($(id).val());
	var groupname = /[`~!@#\$%\^\&\*\(\)_\+<>\?:"\{\},\.\\\/;'\[\]]/im;//特殊字符正则    
	
	if (str === "") {
		$(errId).text(tipRequire);
		return false;
	} else if(groupname.test(str)) {
		$(errId).text(tipSpecial);
		return false;
	}		
...
//判断IO接口数字和字母
function checkInputIo(id, errId, minlength, maxlength, tipRequire, tipRegx, tipSpecial) {
	var str = $.trim($(id).val());
	var groupname = /[`~!@#\$%\^\&\*\(\)\+<>\?:"\{\},\.\\\/;'\[\]]/im;//特殊字符正则    
...
//检查密码，允许密码为空白字符
function checkPasswordInput(id, errId, emptyTip,tipSpecial) {
	var pwd = $(id).val();
	var groupname = /[`~!@#\$%\^\&\*\(\)_\+<>\?:"\{\},\.\\\/;'\[\]]/im;//特殊字符正则    
	if (groupname.test(pwd)) {
		$(errId).text(tipSpecial);
		return false;
	} else if(pwd != "") {
		$(errId).text("");
...
function showErrorMessage(result) {
	...
	case 39:	//RET_DEVICE_IDNO_USED = 39;	//设备编号已经被使用
		...
		break;
	case 41:	//RET_IMAGE_SIZE_ERR = 41;	//图片大小超过1M
		alert(top.lang.errImageSize);
		break;
	case 42:	//RET_IMAGE_TYPE_ERR = 42;	//图片格式错误
	...
	case 56:	//RET_MARK_USED = 56; //标记已被使用
		...
		break;
	case 57:		//车辆信息已经存在
		alert(top.lang.errVehicelHasUsed);
		break;
	case 58:		//司机信息已经存在
		alert(top.lang.errDriverExists);
		break;
	case 59:		//添加车辆不能超过注册车辆
		alert(top.lang.errDeviceExceedRegistered);
		break;
	case 666:		//添加车辆不能超过注册车辆
		alert(top.lang.vehi_school_err);
		break;
	case 667:		//添加车辆不能超过注册车辆
		alert(top.lang.vehi_student_err);
		break;
	default:	//未知错误
...
function getAlarmTypeString(armType) {
	...
	case 34:		
		...
		break;
	case 49:		
		ret = parent.lang.alarm_type_driver_fatigue;
		break;
	case 113:		
