...
function updateVehicleEx(vehicle, jindu, weidu, huangXiangId, statusImage, speed, time, statusStr)
{
	...
	var vehiText = getTxtByVehicle(vehicle);
    var vehiName = vehicle.getName();
    ...
    if (null == vehicle.popMarker)
    {
        ...
        var popMarker=new L.Marker(point, markerOptions);
		
        popMarker.bindPopup(vehiText).openPopup();
		popMarker.bindLabel(vehiName, { noHide: false });
        popMarker.addTo(GFRAME.map);
		vehicle.popMarker = popMarker;
    }
    else
    {
		vehicle.popMarker.setLatLng(point);
		vehicle.popMarker.bindPopup(vehiText).closePopup();
		vehicle.popMarker.bindLabel(vehiName, { noHide: false });
		vehicle.popMarker.update();
    }
...
{		//解析标记点信息
    ...
    } else
    {
        ...
        if (typeId == 2)
        {
			...
			var p1=L.latLng(lngList[0], latList[0]),p2= L.latLng(lngList[1], latList[1]);
