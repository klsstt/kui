...
mapframe.prototype.mapMouseMove = function(event)
{
    if (!GFRAME.isMarkerEdit())
    {
        ...
        if (GFRAME.vehicleList.size() <= 100)
        {
            ...
            var markpoint = GFRAME.map.panTo(latlng); //overlay.getProjection().fromLatLngToDivPixel(latlng);
            ...
            GFRAME.vehicleList.each(function makeShow(vehiId, vehicle)
            {
                try
                {
                    if (num <= 10)
                    {
                        ...
                        {
                            var point = GFRAME.map.panTo([vehicle.getJindu(), vehicle.getWeidu()]);
...
mapframe.prototype.mapMouseClick = function (event)
{
    if (!GFRAME.isMarkerEdit())
    {
        ...
        var minTemp = null;
        var latlng = event.latlng;
        var point = GFRAME.map.panTo(latlng);
        var vehicle = null;
        GFRAME.vehicleList.each(function findMinDistance(vehiId, vehicle)
        {
            try
            {
                var markpoint = GFRAME.map.panTo([vehicle.getJindu(), vehicle.getWeidu()]);
