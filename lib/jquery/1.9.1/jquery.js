...
(function(window, undefined) {
    ...
    //不能这样做，因为包括ASP.NET跟踪几个应用程序
    //通过arguments.caller.callee和Firefox堆栈如果死亡
    //你试图通过“使用严格”的呼叫链进行追踪。（＃13335）
    //支持：Firefox 18+
    //"use strict";
    var
    //在DOM上准备使用延迟
    ...
    // jQuery.Event 基于ECMAScript语言绑定指定的DOM3事件
    ...
    /*处理对ajax请求的响应：
     * - 相应地设置所有responseXXX字段
     * - 找到正确的数据类型（在内容类型和期望的数据类型之间进行调解）
     * - 返回相应的响应
     */
    ...
    //将jQuery公开为AMD模块，但仅适用于AMD加载器
         //了解加载多个jQuery版本的问题
         //在所有可能调用define（）的页面中。 装载机将显示
         //他们对多个jQuery版本有特殊的限制
         //指定define.amd.jQuery = true。 注册为命名模块，
         //由于jQuery可以与可能使用define的其他文件连接，
         //但不能使用理解匿名的适当连接脚本
         // AMD模块。 名为AMD是最安全和最稳健的注册方式。
         //使用小写jquery，因为AMD模块名称是从中派生的
         //文件名，jQuery通常以小写文件名交付。
         //在创建全局之后执行此操作，以便AMD模块想要调用
         // noConflict隐藏这个版本的jQuery，它会工作。
