...
(function($) {
	...
	$.myajax = {
		...
		showPagination: function(id) {
			...
			pagination.push("			<a id=\"homePage\" href=\"javascript:void(0)\">" + parent.lang.homePage + "</a>");
			pagination.push("			&nbsp;&nbsp;");
			pagination.push("			<a id=\"previousPage\" href=\"javascript:void(0)\">" + parent.lang.prevPage + "</a>");
			pagination.push("			&nbsp;&nbsp;");
			pagination.push("			<a id=\"nextPage\" href=\"javascript:void(0)\">" + parent.lang.nextPage + "</a>");
			pagination.push("			&nbsp;&nbsp;");
			pagination.push("			<a id=\"endPage\" href=\"javascript:void(0)\">" + parent.lang.endPage + "</a>");
			pagination.push("			&nbsp;&nbsp;");
			...
			pagination.push("			&nbsp;&nbsp;"+parent.lang.pernumber+" <select id=\"pageRecords\" style=\"width:60px;\" name=\"pageRecords\"><option value='10'>10</option><option value='50'>50</option><option value='100'>100</option> </select>");
		...
		initPagination : function(action, pagination, checkcallback, ajaxcallback, param) {
			...
			$(pagination.id).find('#pageRecords').val(pagination.pageRecords);	
			//当前页码
			$(pagination.id).find('#pageRecords').unbind('change').bind("change", pagination, function() {
				if (!checkcallback.call(null, true)) {
					...
					//判断当前页码
					var pageRecords = $(pagination.id).find('#pageRecords').val();
					if (pageRecords <= 0){
						alert(parent.lang.pageZoneUnvalid);
						return ;
					}
					pagination.pageRecords = Number(pageRecords);
			...
			// 首页
			$(pagination.id).find('#homePage').unbind('click').bind("click", pagination, function() {
				if (!checkcallback.call(null, true)) {
					return ;
				}
				...
				if (pagination.hasPreviousPage) {
					...
					$.myajax.showLoading(true);
					pagination.currentPage = Number("1");
					pagination.totalRecords=$(pagination.id).find('#totalRecords').val();
					if (typeof param != "undefined") {
						$.myajax.jsonGetEx(action, ajaxcallback, pagination, param);
					} else {
						$.myajax.jsonGet(action, ajaxcallback, pagination);
			...
			// 尾页
			$(pagination.id).find('#endPage').unbind('click').bind("click", pagination, function() {
				if (!checkcallback.call(null, true)) {
					return ;
				}
