...
function gpsGetPosition(jingDu, weiDu) {
	return weiDu + "," + jingDu;
	
}
function gpsGetLiCheng(licheng) {	
	if (licheng !== null && licheng >= 1) {
		return (licheng / 1000).toFixed(3);
	} else {
		return "0";
	}
}
function gpsGetLiChengKm(licheng) {	
	if (licheng !== null && licheng >= 0) {
		return licheng.toFixed(1);
	} else {
		return "0";
	}
}
//青岛计价器设计有问题，单位是一公里，但是实际为100m所以要里程要缩短10倍
function gpsGetLiChengHus(licheng) {	
	if (licheng !== null && licheng >= 0) {
		return (licheng / 10).toFixed(3);
	} else {
		return "0";
	}
}
function gpsGetYingLiLiCheng(licheng) {	
	if (licheng !== null && licheng >= 1) {
		return (licheng / 1000 * 0.6213712).toFixed(3);
	} else {
		return "0";
	}
}
...
function gpsGetYingliSpeed(speed, status1) {
	if (gpsIsGpsValid(status1)) {
		return (speed / 10 * 0.6213712).toFixed(2);
	} else {
		return 0.0;
	}
}
...
function getTempSensor(temp) {
	return (temp / 10).toFixed(1);
}
function getTempHuashiSensor(temp) {
	return (temp*9/50+32).toFixed(1);//华氏度℃×9/5+32
}

function getTempHuashiSensor2(temp) {
	return (temp*9/5+32).toFixed(1);//华氏度℃×9/5+32
}
function getTempSensorUnit() {
		return "°C";	
}
function getTempSensorHuashiUnit() {
	return "°F";	
}
...
function gpsGetTimeUnit(isTime) {
	if (isTime) {
		return "s";
	} else {
		return "mini";
...
function gpsGetLabelUnit(isMeter) {
	if (isMeter) {
		return "km";
//			return "Foot";
	} else {
...
function gpsGetLableSpeedUnit(isMeter) {
	if (isMeter) {
		return "km/h";
//			return "Foot";
	} else {
...
function gpsGetLableFuelUnit(isMeter) {
	if (isMeter) {
		return "L";
//			return "Foot";
	} else {
...
function gpsGetLabelYingliLiChengUnit() {
	return parent.lang.labelLiChengMile;
	//parent.lang.milePerHour;
}
...
function gpsGetJindu(jindu) {
	if (jindu !== null) {
		return jindu * 1000000;
	} else {
		return 0;
	}
	
}
function gpsGetWeidu(weidu) {
	if (weidu !== null) {
		return weidu * 1000000;
	} else {
		return 0;
	}
...
function gpsGetChannelName_new(chnName, chnIndex) {
	...
	if(chnIndex==""){chnIndex=0;}
	if (ret === "" || ret =="undefined") {
		ret = parent.lang.channel + (parseInt(chnIndex) + 1);
	}
	return ret;
}
...
function getTimeDifference(second) {
	...
	if(hours != 0) {
		difValue += " " + hours + ' ' + parent.lang.report_park_hour;
	}
	if(minutes != 0) {
		difValue += " " + minutes + ' ' + parent.lang.report_park_minute;
	}
	if(seconds != 0) {
		difValue += " " + seconds + ' ' + parent.lang.report_park_second;
	}
	return difValue;
}
...
function gpsParseRealStatus(vehicle, status, timeStr) {
	...
	var sdString=parent.lang.labelSpeed  + gpsGetSpeedString(status.speed, status.status1) + '(' + gpsGetHuangXiangString(status.huangXiang) + ')';
...
function gpsParseDeviceStatus(vehicle, status) {
	...
	data.speedString=info.sdString;

	if (status.online) {
		data.isGpsValid=true;//在线
		data.mapJingDu=status.mapJingDu;
		data.mapWeiDu=status.mapWeiDu;
		data.speed=status.speed;
		data.gpsTime=status.gpsTime;
		data.liCheng=status.liCheng;
		data.position=status.mapJingDu+","+status.mapWeiDu;
		if (info.isAlarm > 0) {
			data.image = 3;	//报警状态
