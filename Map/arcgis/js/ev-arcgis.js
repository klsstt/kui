...
function getSouthWestAndNorthEastFromPolygon(polygon)
{
    var ring = polygon.rings[0];
    var len = ring.length;

    var pt = polygon.getPoint(0, 0);

    var minlat = pt.getLatitude().toFixed(6);
    var maxlat = minlat;
    var minlng = pt.getLongitude().toFixed(6);
    var maxlng = minlng;
    ...
    for (var i = 0; i < len; i++)
    {
        ...
        if (pt)
        {
            lat = pt.getLatitude().toFixed(6);
            if (minlat > lat)
                minlat = lat;
            if (maxlat < lat)
                maxlat = lat;

            lng = pt.getLongitude().toFixed(6);
            if (minlng > lng)
                minlng = lng;
            if (maxlng < lng)
                maxlng = lng;
    ...
    var slat = minlat.toString() + "," + maxlat.toString();
    var slng = minlng.toString() + "," + maxlng.toString();
...
mapframe.prototype.mapAddMarkerPolygonFinish = function (marker_type)
{
    ...
    var temp = null;
    if (2==marker_type || 4 == marker_type ) //画矩形或者搜索
    {
        temp = getSouthWestAndNorthEastFromPolygon(geometry);
    }
    else
    {
        temp = getLatLngStringFromPolygon(geometry);
    }
